...
const MerchantsManage = ()=>{
    const [users, setUsers] = useState([]); 
    const [isFormOpen, setIsFormOpen] = useState(false);
    const [searchTerm, setSearchTerm] = useState(''); // State lưu giá trị từ TextField
    ...
    // Hàm để đóng form khi người dùng hủy hoặc hoàn thành form
    ...
    const fetchUsers = async () => {
      ...
      try {
          const response = await axios.get('http://localhost:5000/api/admin/getAllMerchants', {
              headers: {
                  'Authorization': `Bearer ${token}` // Thêm token vào header
              }
          });
          setUsers(response.data.merchants); // Chỉnh sửa thành merchants
          console.log(response.data.merchants); // Kiểm tra dữ liệu
      } catch (error) {
  ...
  const handleEdit = (id) => {
    ...
    const handleDelete = (id) => {
      if (window.confirm("Are you sure you want to delete this user?")) {
          const token = localStorage.getItem('token'); // Lấy token từ localStorage
          axios.delete(`http://localhost:5000/api/admin/deleteMerchants/${id}`, {
              headers: {
                  'Authorization': `Bearer ${token}` // Thêm token vào header
              }
          })
  ...
  const handleUpdate = (e) => {
    ...
    axios.put(`http://localhost:5000/api/admin/updateMerchants/${editingUserId}`, editFormData, {
      headers: {
        'Authorization': `Bearer ${token}` // Thêm token vào header
      }
    })
    ...
    // Lọc users dựa trên searchQuery (chỉ thực hiện sau khi nhấn nút Search)
...
export default MerchantsManage;